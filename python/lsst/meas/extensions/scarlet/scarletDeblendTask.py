...
class ScarletDeblendContext:
    ...
    def build(
        ...
        # (the observed image with the model subtracted).
        # This is used to prevent neighboring blends from
        # interfering with each other during deblending.
        yx0 = observation.images.yx0
        deconvolved = scl.Image(mDeconvolved.image.array, bands=observation.images.bands, yx0=yx0)
        convolved = utils.multiband_convolve(deconvolved.data, observation.psfs)
        residual = observation.images.data - convolved
        residualImage = scl.Image(residual, bands=observation.images.bands, yx0=yx0)
...
def deblend(
    ...
    residual = context.residual[:, bbox].copy()
    footprintData = footprint.spans.asArray()
    data = context.deconvolved[:, bbox].data * footprintData

    # Insert the data for the current blend into the residual image
    data_image = scl.Image(data, bands=observation.images.bands, yx0=bbox.origin)
    convovled_data = observation.convolve(data_image).data
    observation.images.data[:] = residual.data + convovled_data
...
class ScarletDeblendTask(pipeBase.Task):
    ...
    @timeMethod
    def run(self, mExposure, mDeconvolved, mergedSources):
        ...
        mDeconvolved : `MultibandExposure`
    ...
    @timeMethod
    def deblend(self, mExposure, mDeconvolved, catalog):
        ...
        dataModel = scl.io.ScarletModelData(context.observation.model_psf)

        nParents = len(catalog)
        nSubParents = 0
        nDeblendedParents = 0
        nDeblendedSubParents = 0
        skippedParents = []
        ...
        for parentIndex in range(nParents):
            ...
            subParent.assign(subParents[0].getFootprint().peaks[0], self.peakSchemaMapper)
            ...
            for subParent in subParents:
                ...
                # Catch all errors and filter out the ones that we know about
                    ...
                    if isinstance(e, ScarletGradientError):
                        subParent.set(self.iterKey, e.iterations)
                    else:
        ...
        self.log.info(
            ...
            f"and split into {nSubParents} sub-blends, of which {nDeblendedSubParents} were deblended,"
